...
static void SeparateOutput() => Console.WriteLine(new string('-', 120));
	...
	double percents, int term, DateTime month, bool recountPercent = false)
{
	...
	while (--term >= 0)
	{
		...
		if (recountPercent)	// we need to recount percent after annuityPayment in this month with specific output
		{
			...
			double nextAnnuityPayment = GetAnnuityPayment(remainingDebt, term, rate);
			double nextPercent = GetPercents(remainingDebt, rate, month);
			Console.WriteLine("{0:dd.MM.yyyy} | Payment: {1,12:N2} | Debt part: {2,12:N2} | Percents: {3,12:N2} | Remaining Debt: {4,12:N2}",
...
double overpayDefault = GetOverpayAfterAddPayment(remainingSum, annuityPayment, rate, percents, term, month) + overpay;
SeparateOutput();
